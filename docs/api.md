...
- `GET /quotes/current-price/{symbol}`：測試查價能力，需目前 provider 支援即時查價（`shioaji_demo`、`fubon`），否則回 503 `QUOTE_PROVIDER_UNAVAILABLE`；`source` 由 provider 自報；`quoteTime` 在尚無成交時為 `null`。不是通用市場資料 API。
