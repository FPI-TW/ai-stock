...
- `src/app/services/quote/fubon/client.py`：`FubonClient(*, personal_id, password, cert_pfx: bytes, cert_password)`，`login()`（temp 檔寫 pfx → `sdk.login` → 過濾 `account_type=="stock"`；失敗對映為 PR1 內定義的安全 `FubonLoginFailureCode` 與不帶 SDK 原文的 `QuoteProviderUnavailableError("fubon")`）、`logout()`、`init_realtime(Mode.Normal)` + `connect`、`subscribe`／`unsubscribe`（維護 `symbol → channel_id`）、`on_disconnect` 只記安全 log 並把 `_realtime_connected` 設為 `False`、`set_on_event` 收 `300`／`301`／`304` 把 `_login_alive` 設為 `False`（`201` 只記 code、`302` 忽略）、`reconnect_realtime()`（重做 `init_realtime` → 重掛 listener → `connect` → 重訂 `_subscribed` 每一檔並重建 channel_id 對照；`init_realtime` 失敗視為登入失效，raise 不帶原文的 `QuoteProviderUnavailableError`）、`get_stock_quote(symbol)`（REST，攔 `FugleAPIError`，429 對映 provider 錯誤，對外 `details` 不回傳 SDK 原文）。
...
- `QuoteSnapshot` 欄位改為 `last_trade_time: datetime | None`；`quote_time` 保留為唯讀 property（有成交時間回它，否則回 `received_at`），validator、觸發、通知沿用 `quote_time` 語意不必逐點改 None 處理。對外 JSON 欄位 `quoteTime` 不改名但映射 `last_trade_time`，可為 `null`。（2026-09-18 實作定案）
