...
- **連線重建迴圈（週期檢查，不做每日固定重登）**：行情 WS 每日收盤後被富邦端斷掉且 SDK 不重連；登入本身實測跨夜不失效（2026-09-17 10:26～09-18 09:50 連續 23.5 小時授權查詢全數成功），但富邦有文件化的登入斷線事件可偵測。兩種斷線都只設旗標，恢復統一交給 pool 一支每 30 秒的週期任務（形狀比照 `IdempotencyCleanupScheduler`）。迴圈只在 `TradingSessionService.is_trading_day(today)` 為真且台北時間 08:30～13:35 內動作，週末與時段外不重連（避免收盤後反覆斷連）：
  - 行情 `on_disconnect` → `_realtime_connected = False`。迴圈看到只做 `reconnect_realtime()`（重做 `init_realtime` → 重掛 listener → `connect` → 重訂 `_subscribed`）。SDK 的 `connect()` 是無 sleep 的 busy-spin（最長約 5 秒等 auth ack），provider 已把它移出 lock，但仍會搶 GIL：迴圈必須用 `asyncio.to_thread(provider.reconnect_realtime)` 在 worker thread 跑，不可直接在 event loop 上呼叫；同一 provider 重入時 `reconnect_realtime()` 直接返回，迴圈不需自己防重入。
